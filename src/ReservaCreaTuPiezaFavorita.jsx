...
const CLASE_ID = "creatupiezafavorita";
const RESERVAS_PATH_KEY = "CreaTuPiezaFavorita";
...
const mapearPrecioDesdeFirebase = (tipoPieza, precios) => {
  if (!tipoPieza || !precios) return 0;

  if (tipoPieza === "cuenco_taza") {
    return Number(precios.cuenco_taza || 0);
  }

  if (tipoPieza === "frutero_grande") {
    return Number(precios.frutero_grande || 0);
  }

  if (tipoPieza === "jarron_grande") {
    return Number(precios.jarron_grande || 0);
  }

  return 0;
};

const getNombreTipoPieza = (tipoPieza) => {
...
export default function ReservaCreaTuPiezaFavorita() {
  ...
  const precioUnitario = useMemo(() => {
    return mapearPrecioDesdeFirebase(tipoPieza, precios);
  }, [tipoPieza, precios]);
  ...
  const nombreTipoPieza = useMemo(() => {
    return getNombreTipoPieza(tipoPieza);
  }, [tipoPieza]);
  ...
  const handleSubmit = async (e) => {
    ...
    if (!fecha || !turno || !metodo || !tipoPieza) {
      alert("Selecciona tipo de pieza, fecha, método y turno.");
      return;
    }
  ...
  const plazasNum = Number(plazas) > 0 ? Number(plazas) : 1;
  const puedeElegirFecha = !!tipoPieza;
  const puedeElegirMetodo =
    !!tipoPieza && !!fecha && !fechaBloqueada && !diaNoDisponible;
  const puedeElegirTurno =
    !!tipoPieza && !!fecha && !!metodo && !fechaBloqueada && !diaNoDisponible;
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              <div>
                ...
                <select
                  id="tipoPieza"
                  value={tipoPieza}
                  onChange={(e) => handleTipoPiezaChange(e.target.value)}
                  className="w-full border border-gray-300 rounded-lg px-3 py-2 text-base"
                  required
                >
                  <option value="">-- Elige una opción --</option>
                  <option value="cuenco_taza">
                    Cuenco o taza — {Number(precios.cuenco_taza || 0)}€
                  </option>
                  <option value="frutero_grande">
                    Frutero / cuenco grande — {Number(precios.frutero_grande || 0)}€
                  </option>
                  <option value="jarron_grande">
                    Jarrón grande — {Number(precios.jarron_grande || 0)}€
                  </option>
                </select>
              </div>
              ...
              <div>
                ...
                {!tipoPieza && (
                  <p className="mt-1 text-xs text-gray-500">
                    Primero elige el tipo de pieza.
                  </p>
                )}
                {fechaBloqueada && (
